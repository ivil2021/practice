...
function renderTodos(doc) {
  ...
  let id = document.createElement("span"); // Creating a new span elements inside li element
  let name = document.createElement("span"); // Creating a new span elements inside li element
  let isDone = document.createElement("span"); // Creating a new span elements inside li element

  // Setting an attribute to each li element in order to use it in future
  li.setAttribute("data-id", doc.id); // id is the autogenerated id from firebase

  id.textContent = doc.data().id; // Setting the text content of each span element
  name.textContent = doc.data().name; // name is a property name from Firebase (for example Learn HTML)
  isDone.textContent = doc.data().isDone; // name is a property name from Firebase (for example Learn HTML)

  //li.appendChild(id); // Appending an element to the list element
  li.appendChild(name);
  //li.appendChild(isDone);

  ////////////////////

  // Creating Del button at the end of the task
  let delBtnDOMElement = document.createElement("button");
  delBtnDOMElement.className = "btn-delete-todo"; // Setting class to the button
  delBtnDOMElement.textContent = "Del"; // Adding text Del to the button

  li.appendChild(delBtnDOMElement); // Adding Del button at the end of the task
  ////////////////////
...
addBtn.addEventListener("click", (e) => {
  ...
  db.collection("MyTodoList").add({
    name: input.value, // getting the value from my input field
    isDone: false,
  });
...
db.collection("MyTodoList").onSnapshot((snapshot) => {
  ...
  changes.forEach((change) => {
    if (change.type === "added") {
      ...
      let todo = new Todo(data.name, data.isDone, change.doc.id); // Creating a new Todo with data from Firebase
      todoList.addElement(todo);
